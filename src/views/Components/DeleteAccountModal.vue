<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-2xl shadow-2xl border border-red-100 w-full max-w-sm overflow-hidden transform transition-all z-10 animate-in fade-in zoom-in-95 duration-200">
      <div class="p-6 text-center space-y-4">
        <div class="w-12 h-12 rounded-full bg-red-50 text-red-600 flex items-center justify-center mx-auto shadow-sm">
        ...
        <div>
          ...
          <p class="text-xs text-slate-500 mt-2 leading-relaxed">
            Bu işlem geri alınamaz. Hesabınız silindiğinde tanımladığınız tüm ürün kartları, kategoriler ve ambar geçmişiniz <strong>kalıcı olarak yok edilecektir.</strong>
          </p>
        </div>
...
const handleDelete = async () => {
  ...
  try {
    ...
    if (response.ok) {
      alert("Hesabınız ve tüm verileriniz başarıyla silindi. Elveda moruq!");
      localStorage.removeItem('token'); // Token'ı uçur
      router.push('/login'); // Kullanıcıyı giriş ekranına postala
    } else {
      const err = await response.json();
      alert(err.message || "Hesap silinirken bir hata meydana geldi.");
    }
  } catch (error) {
