      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
        <div>
          ...
          <input 
            ...
            type="text" required :disabled="isLoading" placeholder="Adınızı ve soyadınızı girin..." 
        ...
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Şirket / İşletme Adı</label>
          <input 
            v-model="formData.companyName" 
            type="text" required :disabled="isLoading" placeholder="Şirket adını girin..." 
            class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm disabled:opacity-60"
          />
        </div>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.ok) {
      alert("Profil bilgileriniz başarıyla güncellendi moruq!");
      emit('success'); // Üst bileşene haber ver (Layout verileri tazeleyecek)
      emit('close');
    } else {
