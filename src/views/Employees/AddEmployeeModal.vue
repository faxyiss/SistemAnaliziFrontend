...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-md overflow-hidden z-10">
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <div class="col-span-2">
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Ad Soyad</label>
          ...
          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Görevi</label>
            <input v-model="form.position" type="text" required :disabled="isLoading"
              placeholder="Örn: Kasiyer"
              class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm disabled:opacity-60" />
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Telefon</label>
            <input v-model="form.phone" type="tel" :disabled="isLoading"
              placeholder="05xx xxx xx xx"
              class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm disabled:opacity-60" />
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">İşe Başlangıç</label>
            <input v-model="form.startDate" type="date" required :disabled="isLoading"
              class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm disabled:opacity-60" />
          </div>

          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Brüt Maaş (₺)</label>
            <input v-model.number="form.grossSalary" type="number" min="0.01" step="0.01" required :disabled="isLoading"
              placeholder="0,00"
              class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm disabled:opacity-60" />
          </div>
        </div>
        ...
        <div v-if="form.grossSalary > 0" class="bg-slate-50 rounded-xl p-4 space-y-2 border border-slate-100">
          <p class="text-xs font-bold text-slate-500 uppercase tracking-wide mb-2">Maaş Hesaplama Önizlemesi</p>
          <div class="flex justify-between text-xs">
            <span class="text-slate-500">SGK (Çalışan %14)</span>
            <span class="font-semibold text-slate-700">-{{ formatCurrency(calc.employeeSGK) }}</span>
          </div>
          <div class="flex justify-between text-xs">
            <span class="text-slate-500">İşsizlik (Çalışan %1)</span>
            <span class="font-semibold text-slate-700">-{{ formatCurrency(calc.employeeUnemployment) }}</span>
          </div>
          <div class="flex justify-between text-xs">
        ...
        <div class="flex items-center justify-end gap-3 pt-2 border-t border-slate-100">
          ...
          <button type="submit" :disabled="isLoading || !form.fullName || !form.position || !form.grossSalary"
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const res = await fetch(`${BASE_URL}/Employees`, {
      ...
      body: JSON.stringify({ ...form.value, startDate: new Date(form.value.startDate).toISOString() }),
    })
