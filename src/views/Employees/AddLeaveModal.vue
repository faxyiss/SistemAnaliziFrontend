...
<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-sm overflow-hidden z-10">
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4">
        ...
        <div v-if="employee" class="flex items-center gap-3 bg-slate-50 rounded-xl p-3 border border-slate-100">
          <div class="w-9 h-9 rounded-xl bg-linear-to-r from-blue-500 to-indigo-500 text-white flex items-center justify-center font-bold text-sm flex-shrink-0">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Başlangıç</label>
          ...
          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Bitiş</label>
            <input v-model="form.endDate" type="date" required :disabled="isLoading"
              :min="form.startDate"
              class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm disabled:opacity-60" />
          </div>
        </div>
        ...
        <!-- Gün hesabı -->
        <div v-if="dayCount > 0" class="flex items-center justify-center gap-2 bg-amber-50 border border-amber-100 rounded-xl py-2.5">
          <CalendarIcon :size="14" class="text-amber-500" />
          <span class="text-sm font-bold text-amber-700">{{ dayCount }} gün izin</span>
        </div>

        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Açıklama (Opsiyonel)</label>
          <textarea v-model="form.reason" rows="2" :disabled="isLoading"
        ...
        <div class="flex items-center justify-end gap-3 pt-2 border-t border-slate-100">
          ...
          <button type="submit" :disabled="isLoading || !form.startDate || !form.endDate"
...
const handleSubmit = async () => {
  ...
  try {
    ...
    else alert(data.message || 'İzin kaydedilemedi.')
