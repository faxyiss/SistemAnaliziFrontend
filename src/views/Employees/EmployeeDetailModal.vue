<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-2xl overflow-hidden z-10 max-h-[90vh] flex flex-col">

      <!-- Header -->
      <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 flex-shrink-0">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-linear-to-r from-blue-500 to-indigo-500 text-white flex items-center justify-center font-bold flex-shrink-0">
      ...
      <div class="flex-1 overflow-y-auto custom-scrollbar">
        ...
        <div v-if="activeTab === 'summary'" class="p-6 space-y-4">
          <div class="grid grid-cols-2 gap-3">
            <div class="bg-slate-50 rounded-xl p-4 border border-slate-100">
              ...
              <p class="text-lg font-bold text-slate-800 mt-1">{{ formatCurrency(employee?.grossSalary) }}</p>
            </div>
            <div class="bg-emerald-50 rounded-xl p-4 border border-emerald-100">
              <p class="text-xs text-emerald-600 font-medium">Net Maaş</p>
              <p class="text-lg font-bold text-emerald-700 mt-1">{{ formatCurrency(employee?.netSalary) }}</p>
            </div>
            <div class="bg-red-50 rounded-xl p-4 border border-red-100">
              <p class="text-xs text-red-500 font-medium">İşveren Maliyeti</p>
              <p class="text-lg font-bold text-red-600 mt-1">{{ formatCurrency(employee?.totalEmployerCost) }}</p>
            </div>
          ...
          <div class="bg-slate-50 rounded-xl border border-slate-100 overflow-hidden">
            <div class="px-4 py-2.5 bg-slate-100/60 border-b border-slate-200">
              <p class="text-xs font-bold text-slate-600 uppercase tracking-wide">Aylık Vergi & SGK Dökümü</p>
            </div>
            <div class="p-4 space-y-2">
              <div v-for="row in taxRows" :key="row.label" class="flex justify-between text-xs">
                <span class="text-slate-500">{{ row.label }}</span>
                <span class="font-semibold" :class="row.color">{{ formatCurrency(row.value) }}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div v-if="activeTab === 'payments'" class="p-6">
          <div v-if="isLoadingPayments" class="py-8 text-center text-slate-400 text-sm">Yükleniyor...</div>
            ...
            <div v-for="p in payments" :key="p.id"
              ...
              <div>
                ...
                <div class="flex items-center gap-3 mt-1">
                  <span class="text-xs text-slate-500">Net: <span class="font-semibold text-emerald-600">{{ formatCurrency(p.netSalary) }}</span></span>
                  <span class="text-xs text-slate-400">|</span>
                  <span class="text-xs text-slate-500">İşv. Maliyet: <span class="font-semibold text-red-500">{{ formatCurrency(p.totalEmployerCost) }}</span></span>
                </div>
        ...
        <div v-if="activeTab === 'leaves'" class="p-6">
          <div v-if="isLoadingLeaves" class="py-8 text-center text-slate-400 text-sm">Yükleniyor...</div>
            ...
            <div v-for="l in leaves" :key="l.id"
              ...
              <div>
                <div class="flex items-center gap-2">
                  <span class="font-bold text-slate-800 text-sm">{{ formatDate(l.startDate) }} — {{ formatDate(l.endDate) }}</span>
                  <span class="px-2 py-0.5 rounded-md bg-amber-50 border border-amber-100 text-amber-700 text-xs font-bold">{{ l.dayCount }} gün</span>
                </div>
                <p v-if="l.reason" class="text-xs text-slate-400 mt-0.5">{{ l.reason }}</p>
              </div>
              <button @click="deleteLeave(l)"
                class="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors flex-shrink-0">
                <Trash2Icon :size="14" />
              </button>
            </div>
...
const fetchPayments = async () => {
  ...
  try {
    ...
    const res = await fetch(`http://31.210.36.10:5000/api/Employees/${props.employee.id}/payments`, {
...
const fetchLeaves = async () => {
  ...
  try {
    const token = localStorage.getItem('token')
    const res = await fetch(`http://31.210.36.10:5000/api/Employees/${props.employee.id}/leaves`, {
      headers: { Authorization: `Bearer ${token}` },
    })
    if (res.ok) leaves.value = await res.json()
  } finally { isLoadingLeaves.value = false }
}

const deletePayment = async (p: any) => {
  if (!confirm(`${p.monthLabel} ödemesini silmek istiyor musunuz?`)) return
  const token = localStorage.getItem('token')
  const res = await fetch(`http://31.210.36.10:5000/api/Employees/payments/${p.id}`, {
...
const deleteLeave = async (l: any) => {
  ...
  const token = localStorage.getItem('token')
  const res = await fetch(`http://31.210.36.10:5000/api/Employees/leaves/${l.id}`, {
    method: 'DELETE', headers: { Authorization: `Bearer ${token}` },
  })
...
watch(() => props.isOpen, (v) => {
  if (v) { activeTab.value = 'summary'; fetchPayments(); fetchLeaves() }
  else   { payments.value = []; leaves.value = [] }
})
</script>
