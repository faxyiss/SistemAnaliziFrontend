<template>
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-md overflow-hidden z-10 max-h-[90vh] flex flex-col">
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-4 overflow-y-auto custom-scrollbar flex-1">

        <!-- Personel Bilgisi -->
        <div v-if="employee" class="flex items-center gap-3 bg-slate-50 rounded-xl p-3 border border-slate-100">
          <div class="w-10 h-10 rounded-xl bg-linear-to-r from-blue-500 to-indigo-500 text-white flex items-center justify-center font-bold text-sm flex-shrink-0">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <select v-model.number="form.month" :disabled="isLoading"
              class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm bg-white disabled:opacity-60">
              <option v-for="m in months" :key="m.value" :value="m.value">{{ m.label }}</option>
            </select>
          </div>
          <div>
            <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Yıl</label>
            <select v-model.number="form.year" :disabled="isLoading"
              class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm bg-white disabled:opacity-60">
        ...
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">
            Brüt Maaş (₺)
            <span class="text-slate-400 font-normal normal-case ml-1">— boş bırakırsanız kayıtlı maaş kullanılır</span>
          </label>
          <input v-model.number="form.grossSalary" type="number" min="0.01" step="0.01" :disabled="isLoading"
        ...
        <div class="bg-slate-50 rounded-xl border border-slate-100 overflow-hidden">
          ...
          <div class="p-4 space-y-2">
            ...
            <div class="border-t border-slate-200 pt-2 space-y-1.5">
              <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Çalışan Kesintileri</p>
              <div class="flex justify-between text-xs">
                <span class="text-slate-500">SGK Primi (%14)</span>
                <span class="font-semibold text-red-500">-{{ formatCurrency(calc.employeeSGK) }}</span>
              </div>
              <div class="flex justify-between text-xs">
                <span class="text-slate-500">İşsizlik Sigortası (%1)</span>
                <span class="font-semibold text-red-500">-{{ formatCurrency(calc.employeeUnemployment) }}</span>
              </div>
              <div class="flex justify-between text-xs">
                <span class="text-slate-500">Gelir Vergisi (%15)</span>
                <span class="font-semibold text-red-500">-{{ formatCurrency(calc.incomeTax) }}</span>
              </div>
              <div class="flex justify-between text-xs">
            ...
            <div class="border-t border-slate-200 pt-2 flex justify-between">
              ...
              <span class="text-sm font-bold text-emerald-700">{{ formatCurrency(calc.netSalary) }}</span>
            </div>
            <div class="border-t border-slate-200 pt-2 space-y-1.5">
              <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wide">İşveren Ek Maliyeti</p>
              <div class="flex justify-between text-xs">
                <span class="text-slate-500">SGK Primi (%20.5)</span>
                <span class="font-semibold text-slate-700">+{{ formatCurrency(calc.employerSGK) }}</span>
              </div>
              <div class="flex justify-between text-xs">
                <span class="text-slate-500">İşsizlik Sigortası (%2)</span>
                <span class="font-semibold text-slate-700">+{{ formatCurrency(calc.employerUnemployment) }}</span>
              </div>
            </div>
            <div class="border-t-2 border-slate-300 pt-2 flex justify-between">
              ...
              <span class="text-sm font-bold text-red-600">{{ formatCurrency(calc.totalEmployerCost) }}</span>
            </div>
          </div>
        </div>
        ...
        <!-- Notlar -->
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Notlar (Opsiyonel)</label>
          <textarea v-model="form.notes" rows="2" :disabled="isLoading"
            placeholder="Prim, kesinti gibi ek notlar..."
            class="w-full px-3 py-2 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all text-sm disabled:opacity-60 resize-none"></textarea>
        </div>
      </form>
...
import { WalletIcon, XIcon, CheckIcon } from 'lucide-vue-next'
...
const defaultForm = () => ({
  ...
  grossSalary: null as number | null, notes: '',
})
...
const RATES = { empSGK: 0.14, empUnemp: 0.01, incomeTax: 0.15, stamp: 0.00759, erSGK: 0.205, erUnemp: 0.02 }

const effectiveGross = computed(() =>
  form.value.grossSalary || props.employee?.grossSalary || 0
)
...
const calc = computed(() => {
  ...
  return {
    employeeSGK: empSGK, employeeUnemployment: empUnemp, incomeTax, stampTax: stamp,
    netSalary: net, employerSGK: erSGK, employerUnemployment: erUnemp,
...
watch(() => props.isOpen, (v) => { if (!v) form.value = defaultForm() })
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const body: any = { month: form.value.month, year: form.value.year, notes: form.value.notes || null }
    ...
    else alert(data.message || 'Ödeme kaydedilemedi.')
