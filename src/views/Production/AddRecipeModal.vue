      ...
      <div class="overflow-y-auto p-6 pb-56 space-y-6 custom-scrollbar flex-1 bg-slate-50/30">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm space-y-4">
          ...
          <select
            ...
            class="bg-slate-50 border border-slate-200 text-slate-900 text-sm rounded-xl focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 focus:bg-white block w-full p-3 outline-none transition-all font-medium"
          >
            <option value="" disabled>Lütfen üretilecek nihai ürünü listeden seçiniz...</option>
            <option v-for="prod in finalProducts" :key="prod.id" :value="prod.id">
              {{ prod.name }} (Ürün No: #{{ prod.productNo }})
            </option>
          </select>
        </div>
