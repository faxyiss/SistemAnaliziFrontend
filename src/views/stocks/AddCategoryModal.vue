<template>
  <div v-if="isOpen" class="fixed inset-0 z-60 flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-xl shadow-2xl border border-slate-100 w-full max-w-sm overflow-hidden transform transition-all z-10">
      <div class="flex items-center justify-between px-5 py-3.5 border-b border-slate-100 bg-slate-50/50">
        ...
        <button @click="emit('close')" class="p-1 rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-colors">
          <XIcon :size="16" />
        </button>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="p-5 space-y-4">
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Kategori Adı</label>
          <input 
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert(data.message || "Kategori oluşturulurken bir hata oluştu.");
    }
  } catch (error) {
