<template>
  <div v-if="isOpen" class="fixed inset-0 z-[60] flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-2xl shadow-2xl border border-slate-100 w-full max-w-sm overflow-hidden transform transition-all z-10 animate-in fade-in zoom-in-95 duration-200">
      
      <div class="flex items-center justify-between px-5 py-4 border-b border-amber-100 bg-amber-50/50">
        <h3 class="text-base font-bold text-amber-800 flex items-center gap-2">
          <MinusCircleIcon :size="18" /> Stok Eksilt (Fire / Zayi)
        </h3>
        <button @click="emit('close')" class="p-1.5 rounded-lg text-amber-400 hover:bg-amber-100 hover:text-amber-600 transition-colors">
          <XIcon :size="18" />
        </button>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="p-5 space-y-4">
        <div class="bg-slate-50 p-3 rounded-xl border border-slate-200 text-sm">
          ...
          <div class="flex justify-between">
            ...
            <span class="font-bold text-slate-700">₺{{ stock?.unitCost?.toLocaleString('tr-TR', { minimumFractionDigits: 2 }) }}</span>
          </div>
        </div>

        <div>
          <label class="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1.5">Eksiltilecek Miktar</label>
          <input
          ...
          <p class="text-[10px] text-slate-400 mt-1.5 leading-tight">
            Sayım eksikliği, çalınma, kırılma veya bozulma gibi durumlarda stoku eşitlemek için kullanın.
          </p>
        </div>

        <div v-if="form.quantity > stock?.remainingQuantity" class="text-xs text-red-500 font-bold flex items-center gap-1">
        ...
        <div class="flex justify-end gap-2 pt-3 border-t border-slate-100 mt-2">
          <button type="button" @click="emit('close')" :disabled="isLoading" class="px-4 py-2 rounded-xl text-sm font-bold text-slate-500 hover:bg-slate-50 transition-colors">İptal</button>
          <button 
            type="submit" 
            :disabled="isLoading || !form.quantity || form.quantity > stock?.remainingQuantity || form.quantity < 1" 
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.ok) {
      alert(result.message || "Stok başarıyla eksiltildi.");
      emit('success');
      emit('close');
    } else {
      alert(result.message || "Eksiltme işlemi başarısız.");
    }
  } catch (error) {
