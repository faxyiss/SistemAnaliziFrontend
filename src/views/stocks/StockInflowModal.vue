  ...
  <div v-if="isOpen" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="bg-white rounded-2xl shadow-xl border border-slate-100 w-full max-w-md overflow-hidden transform transition-all z-10">
      <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
        ...
        <button @click="emit('close')" class="p-1.5 rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600 transition-colors">
          <XIcon :size="18" />
        </button>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="p-6 space-y-5">
        <div class="p-3 bg-slate-50 border border-slate-100 rounded-xl flex items-center gap-3">
          ...
          <div v-if="product">
            ...
            <div class="font-bold text-slate-700 text-sm truncate max-w-[250px]">{{ product.name }}</div>
          </div>
        </div>

        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Giriş Miktarı (Adet)</label>
          <input 
            v-model.number="form.inflowQuantity" 
            type="number" min="1" step="1" required :disabled="isLoading" 
            placeholder="Örn: 100" 
        ...
        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">Birim Maliyet (₺)</label>
          <div class="relative">
            <span class="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 font-medium">₺</span>
            <input 
              v-model.number="form.unitCost" 
              type="number" min="0.01" step="0.01" required :disabled="isLoading" 
          ...
          <p class="text-[10px] text-slate-400 mt-1">Bu fiyat satış fiyatı değil, depoya alış/üretim maliyetinizdir.</p>
        </div>

        <div>
          <label class="block text-xs font-semibold text-slate-500 uppercase tracking-wide mb-1.5">İşlem Tarihi (Opsiyonel)</label>
          <input 
            v-model="form.inflowDate" 
            type="datetime-local" :disabled="isLoading" 
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100 mt-6">
          ...
          <button type="submit" :disabled="isLoading" class="px-5 py-2.5 rounded-xl text-sm font-medium bg-emerald-600 hover:bg-emerald-700 text-white shadow-sm shadow-emerald-200 transition-colors flex items-center gap-2 disabled:opacity-60">
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (response.ok) {
      alert(data.message || "Stok başarıyla eklendi.");
      emit('success');
      emit('close');
    } else {
      alert(data.message || "Stok eklenirken bir hata oluştu.");
    }
  } catch (error) {
